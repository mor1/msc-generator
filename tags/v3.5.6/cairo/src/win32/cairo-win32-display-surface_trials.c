					   ...
					   const cairo_rectangle_int_t   *extents)
{
    ...
    cairo_status_t status;
    /* Zozo */
    cairo_rectangle_int_t shifted_extents;
    cairo_image_surface_t *ret;
    /*Zozo end */
    ...
    /*Zozo Added*/
    cairo_surface_set_device_offset(surface->fallback, -surface->win32.extents.x, -surface->win32.extents.y);
    cairo_surface_set_device_offset(surface->image, -surface->win32.extents.x, -surface->win32.extents.y);
    /*Zozo end*/
    }
    ...
    shifted_extents.x = 0;
    shifted_extents.y = 0;
    shifted_extents.width = extents->width;
    shifted_extents.height = extents->height;
    ret = _cairo_surface_map_to_image (surface->image, &shifted_extents);
    cairo_surface_set_device_offset(&ret->base, -surface->win32.extents.x, -surface->win32.extents.y);
    return ret;
    /* original 
...
_cairo_win32_display_surface_flush (void *abstract_surface, unsigned flags)
{
	...
	if (damage->status) {
		...
		if (!BitBlt (surface->win32.dc,
		         /* Zozo modification */
                 surface->win32.extents.x + rect.x, 
                 surface->win32.extents.y + rect.y,
                 /* original below
			     rect.x, rect.y,
                 /*Zozo ends */
			     rect.width, rect.height,
