...
private:
    ...
    result_t UnionIntersectXor(const Contour &b, Contours &result, operation_t operation) const;
	result_t Untangle(Contours &result, operation_t rule) const;
...
protected:
    friend class Contours; //to access the (std::vector<Edge> &&) constructor & GetInverse
    ...
    result_t Union(const Contour &b, Contours &result) const {return UnionIntersectXor(b, result, COMBINE_UNION);}
    result_t Intersect(const Contour &b, Contours &result) const {return UnionIntersectXor(b, result, COMBINE_INTERSECT);}
    result_t Substract(const Contour &b, Contours &result) const {return UnionIntersectXor(b.CreateInverse(), result, COMBINE_INTERSECT);}
    result_t Xor(const Contour &b, Contours &result) const {return UnionIntersectXor(b, result, COMBINE_XOR);}
    result_t Untangle(Contours &result, bool nonzero=true) const {return Untangle(result, nonzero?WINDING_NONZERO:WINDING_EVENODD);}
    void DoVerticalCrossSection(double x, DoubleMap<bool> &section, bool add) const;
    void Expand(EExpandType type, double gap, Contours &res) const;
public:
    ...
    Contours CreateExpand(double gap, EExpandType type=EXPAND_MITER) const;
